...
class MatMulOp(Op):
    ...
    def compute(self, node, input_vals):
        ...
        assert len(input_vals) == 2
        val_a = input_vals[0].transpose() if node.matmul_attr_trans_A else input_vals[0]
        val_b = input_vals[1].transpose() if node.matmul_attr_trans_B else input_vals[1]
...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order:
        ...
        grad_contribs = node.op.gradient(node, node_to_output_grad[node])

        if grad_contribs is not None:
            for (grad_contrib, input_node) in zip(grad_contribs, node.inputs):
                if input_node not in node_to_output_grads_list:
                    node_to_output_grads_list[input_node] = []
                node_to_output_grads_list[input_node] += [grad_contrib]
